...
func NewRouter(
        ...
        protected.Use(middleware.Auth(jwtSecret))
        {
                ...
                // ── Document Masters (super_admin + admin_bpp) ───────────────────────
                docMasters := protected.Group("")
                docMasters.Use(middleware.RequireRole("super_admin", "admin_bpp"))
                {
                        docMasters.POST("/document-kinds", documentHandler.CreateKind)
                        docMasters.PATCH("/document-kinds/:id", documentHandler.UpdateKind)
                        docMasters.DELETE("/document-kinds/:id", documentHandler.DeleteKind)
                        docMasters.POST("/document-categories", documentHandler.CreateCategory)
                        docMasters.PATCH("/document-categories/:id", documentHandler.UpdateCategory)
                        docMasters.DELETE("/document-categories/:id", documentHandler.DeleteCategory)
                        docMasters.POST("/document-types", documentHandler.CreateType)
                        docMasters.PATCH("/document-types/:id", documentHandler.UpdateType)
                        docMasters.DELETE("/document-types/:id", documentHandler.DeleteType)
                }
