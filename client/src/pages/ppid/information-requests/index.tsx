...
import { FileQuestion, Eye, FileSpreadsheet, Search, X } from "lucide-react";
...
export default function PpidInfoRequestsPage() {
  ...
  async function handleExport() {
    setExporting(true);
    try {
      const res = await fetch("/api/admin/export/ppid-info-requests", {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
      if (!res.ok) throw new Error(await res.text());
      const blob = await res.blob();
      const cd = res.headers.get("Content-Disposition") || "";
      const filename = cd.match(/filename="([^"]+)"/)?.[1] || "ppid-permohonan-informasi.xlsx";
  ...
  const { data, isLoading } = useQuery<{ items: InfoRequest[]; total: number }>({
    queryKey: ["/api/admin/ppid/information-requests", {
      page: String(page),
      limit: "20",
    ...
    <div className="flex flex-col gap-6 p-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <FileQuestion className="w-6 h-6 text-blue-500" /> Permohonan Informasi
          </h1>
          <p className="text-muted-foreground text-sm mt-1">{total} permohonan masuk</p>
        </div>
      ...
      <form onSubmit={handleSearch} className="flex gap-2">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            className="pl-9 pr-8"
            placeholder="Cari nama, NIK, telepon, kode..."
            value={searchInput}
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-36">Cara Mendapatkan</TableHead>
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={6} className="text-center py-12 text-muted-foreground">
                  {search ? `Tidak ada hasil untuk "${search}"` : "Belum ada permohonan informasi masuk"}
                </TableCell>
              </TableRow>
            ) : (
              items.map(item => (
                <TableRow key={item.id} data-testid={`row-inforeq-${item.id}`}>
                  <TableCell>
                  ...
                  <TableCell className="text-sm max-w-xs">
                    <p className="truncate">{item.informationDetail}</p>
                  </TableCell>
                  ...
                  <TableCell>
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => setLocation(`/ppid/information-requests/${item.id}`)}
